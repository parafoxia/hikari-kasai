...
"url":8,
"doc":"Joins the given Twitch channels' chats. Example    -   >>> await bot.twitch.join(\"parafoxia\", \"twitchdev\")   Example    -   >>> channels = (\"parafoxia\", \"twitchdev\") >>> await bot.twitch.join( channels)   Parameters       channels : str The login usernames of the channels to join. Returns    - None",
"func":1
},
{
"ref":"kasai.twitch.TwitchClient.part",
"url":8,
"doc":"Parts (leaves) the given Twitch channels' chats.  note You do not need to part joined channels before shutting the bot down \u2014 this is handled automatically. Example    -   >>> await bot.twitch.part(\"parafoxia\", \"twitchdev\")   Example    -   >>> channels = (\"parafoxia\", \"twitchdev\") >>> await bot.twitch.part( channels)   Parameters       channels : str The login usernames of the channels to part. Returns    - None",
"func":1
},
{
"ref":"kasai.twitch.TwitchClient.create_message",
"url":8,
"doc":"Send a message to a Twitch channel. Example    -   >>> await bot.twitch.create_message( \"parafoxia\", \"Never gonna give you up!\", )   Parameters      channel : str The login username of the channel you want to send a message to. content : str The text content of the message you want to send. Returns    - None",
"func":1
},
{
"ref":"kasai.twitch.TwitchClient.fetch_user",
"url":8,
"doc":"Fetches a user from the Twitch Helix API. Example    -   >>> user1 = await bot.twitch.fetch_user(\"parafoxia\") >>> user2 = await bot.twitch.fetch_user(\"606074029\") >>> user1  user2 True   Parameters      user : str The login username or the ID of the user to fetch. Note that while Twitch user IDs are numerical, they are strings. Returns    - kasai.User The fetched user.",
"func":1
},
{
"ref":"kasai.twitch.TwitchClient.fetch_channel",
"url":8,
"doc":"Fetches a channel from the Twitch Helix API. Example    -   >>> channel = await bot.twitch.fetch_channel(\"606074029\") >>> print(channel.username) parafoxia   Parameters      channel : str The ID of the channel to fetch. Note that while Twitch channel IDs are numerical, they are strings. A channel's ID is identical to the user ID of the channel. Returns    - kasai.Channel The fetched channel.",
"func":1
},
{
"ref":"kasai.messages",
